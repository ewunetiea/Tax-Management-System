...
@Mapper
public interface LongOutstandingItemsReviewerMapper {

    @Select("select ima.* from IS_Management_Audit ima " + 
				"INNER JOIN long_outstanding_item_mgt lom on ima.id = lom.mgt_audit_id " + 
				"where ima.category = #{category} and ima.management = #{banking} and ima.review_status = 0 and ima.auditor_status = 1 and status = 1  Order By ima.passed_date DESC ")
	@Results(value = {
    ...
    @Select("select ima.* from IS_Management_Audit ima " + 
				"INNER JOIN long_outstanding_item_mgt lom on ima.id = lom.mgt_audit_id " + 
				"where ima.review_status = 1 and ima.reviewer_id=#{user_id} and ima.status = 1 and ima.approve_status = 0  Order By ima.reviewed_date DESC ")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
			@Result(property = "reviewer", column = "reviewer_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
