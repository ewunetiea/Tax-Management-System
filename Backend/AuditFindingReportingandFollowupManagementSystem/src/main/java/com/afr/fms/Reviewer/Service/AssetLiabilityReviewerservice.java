...
import com.afr.fms.Reviewer.Mapper.AssetLiabilityReviewerMapper;
   ...
   private  AssetLiabilityReviewerMapper assetLiabilityReviewerMapper;
       public List<AuditISM> getFindingBasedOnStatus(PaginatorPayLoad paginatorPayload) {
        ...
        if (paginatorPayload.getAudit_status().equalsIgnoreCase("pending")) {
            IsMgtAudit = assetLiabilityReviewerMapper.getPendingAudits(paginatorPayload);
        } else if (paginatorPayload.getAudit_status().equalsIgnoreCase("reviewed")) {
            IsMgtAudit = assetLiabilityReviewerMapper.getReviewedAudits(paginatorPayload);
        }
        else if (paginatorPayload.getAudit_status().equalsIgnoreCase("rejected")) {
            IsMgtAudit = assetLiabilityReviewerMapper.getApproverRejectedFindings(paginatorPayload);
        }
