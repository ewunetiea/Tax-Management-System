...
@Component({
  ...
  imports: [CommonModule, MultiSelectModule, TableModule, FormsModule, ButtonModule, ToolbarModule, ToastModule, CardModule, TimeagoModule, CalendarModule, BreadcrumbModule, DropdownModule, TextareaModule, InputTextModule, DatePicker],
      ...
      if (u_activity.user?.roles) {
        for (const uactivity of u_activity.user?.roles) {
          if (uactivity.name?.includes('ROLE_ADMIN')) {
            ...
            row['number'] = i + 1;
            row['full_name'] =
              u_activity.user?.first_name + ' ' + u_activity.user?.last_name;
            row['message'] = u_activity?.message;
            row['date_created'] = this.date_format.dateFormatter(
              u_activity?.created_date
            );

            row2['Full Name'] =
              u_activity.user?.first_name + ' ' + u_activity.user?.last_name;
            row2['Message'] = u_activity?.message;
            row2['Date Created'] = this.date_format.dateFormatter(
              u_activity?.created_date
            );
            data[i] = row;
            this.data2[i] = row2;
