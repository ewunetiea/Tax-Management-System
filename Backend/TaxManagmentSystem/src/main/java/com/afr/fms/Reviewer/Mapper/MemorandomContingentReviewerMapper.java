...
@Mapper
public interface MemorandomContingentReviewerMapper {


	@Select("select * from IS_Management_Audit where id in (SELECT is_mgt_audit_id from  memorandom_contingent_mgt)  AND category = #{category} and management = #{banking} and review_status = 0 and auditor_status = 1 and status = 1  Order By passed_date DESC ")
	@Results(value = {
	...
	@Select("select * from IS_Management_Audit where id in (SELECT is_mgt_audit_id from  memorandom_contingent_mgt) AND  review_status = 1 and reviewer_id=#{user_id} and status = 1 and approve_status = 2  Order By approver_rejected_date DESC")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "IS_MGTAuditee", column = "id", many = @Many(select = "com.afr.fms.Auditor.Mapper.AuditISMMapper.getISMAuditees")),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
