...
import com.afr.fms.Auditor.Mapper.AbnormalBalanceAuditorMapper;
...
public class AbnormalBalanceAuditorService {
    ...
    public void createAbnormalBalanceFinding(AuditISM audit) {
        ...
        audit.getAbnormalBalance().setIs_management_audit_id(audit_id);
        abnormalBalanceAuditorMapper.createAbnormalBalance(audit.getAbnormalBalance());
    ...
    public void updateAbnormalBalanceinding(AuditISM audit) {
        User user = userMapper.getAuditorById(audit.getAuditor().getId());
        if (audit.getFinding_detail() == null) {
            abnormalBalanceAuditorMapper.updateMGTFinding(audit);
            abnormalBalanceAuditorMapper.updateAbnormalBalance(audit.getAbnormalBalance());
        } else {
            abnormalBalanceAuditorMapper.updateMGTFindingandFindingDetail(audit);
            abnormalBalanceAuditorMapper.updateAbnormalBalance(audit.getAbnormalBalance());
        }
    ...
    public List<AuditISM> getAuditsOnDrafting(PaginatorPayLoad paginatorPayload) {
        ...
        List<AuditISM> inspectionAudits = abnormalBalanceAuditorMapper.getAuditsOnDrafting(paginatorPayload);
    ...
    public List<AuditISM> getAuditsOnProgress(PaginatorPayLoad paginatorPayload) {
        // PageHelper.startPage(paginatorPayload.getCurrentPage(), paginatorPayload.getPageSize());
        List<AuditISM> inspectionAudits = abnormalBalanceAuditorMapper.getAuditsOnProgress(paginatorPayload);
        // if (!inspectionAudits.isEmpty()) {
        //     Page<AuditISM> page = (Page<AuditISM>) inspectionAudits;
        //     inspectionAudits.get(0).setTotal_records_paginator(page.getTotal());
        // }
    ...
    public List<AuditISM> getRejectedFindings(PaginatorPayLoad paginatorPayload) {
        // PageHelper.startPage(paginatorPayload.getCurrentPage(), paginatorPayload.getPageSize());
        List<AuditISM> inspectionAudits = abnormalBalanceAuditorMapper.getRejectedFindings(paginatorPayload);
        // if (!inspectionAudits.isEmpty()) {
        //     Page<AuditISM> page = (Page<AuditISM>) inspectionAudits;
        //     inspectionAudits.get(0).setTotal_records_paginator(page.getTotal());
        // }
