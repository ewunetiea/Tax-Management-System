...
import com.afr.fms.Auditor.Mapper.MemorandomContingentAuditorMapper;
...
@Service
public class MemorandomContingentService {
    ...
    private MemorandomContingentAuditorMapper memorandomContingentMapper;
    ...
    public List<AuditISM> getFindingBasedOnStatus(PaginatorPayLoad paginatorPayload) {
        ...
        if (paginatorPayload.getAudit_status().equalsIgnoreCase("drafted")) {
            IsMgtAudit = memorandomContingentMapper.getDraftedMemorandomContingent(paginatorPayload);
        } else if (paginatorPayload.getAudit_status().equalsIgnoreCase("passed")) {
          IsMgtAudit = memorandomContingentMapper.getPassedMemorandomContingent(paginatorPayload);
        } 
        else if (paginatorPayload.getAudit_status().equalsIgnoreCase("rejected")) {
           IsMgtAudit = memorandomContingentMapper.getRejectedAuditsForAuditor(paginatorPayload);
        }
    ...
    public void createMemorandomContingent(AuditISM audit) {
        ...
        Long audit_id = (long) 0;
            ...
            audit.getMemorandomAndContingent().setIs_mgt_audit_id(audit_id);
            memorandomContingentMapper.createMemorandomContingent(audit.getMemorandomAndContingent());
    ...
    public void updateMemorandomAndConingent(AuditISM audit) {
        User user = userMapper.getAuditorById(audit.getAuditor().getId());
        ...
        if (audit.getCategory() == "IS") {
            ...
            memorandomContingentMapper.updateMemorandomContingent(audit.getMemorandomAndContingent());
        } else {
