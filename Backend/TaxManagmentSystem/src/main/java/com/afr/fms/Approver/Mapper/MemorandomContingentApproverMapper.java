...
@Mapper
public interface MemorandomContingentApproverMapper {


    
	@Select("select * from IS_Management_Audit where id in (SELECT is_mgt_audit_id from  memorandom_contingent_mgt)   AND category = #{category} and review_status = 1  and approve_status = 0  and status = 1  Order By reviewed_date DESC  ")
	@Results(value = {
    ...
    // @Select("select * from IS_Management_Audit where  id in (SELECT is_mgt_audit_id from  memorandom_contingent_mgt)  AND review_status = 1 and reviewer_id=#{user_id} and status = 1 and approve_status = 0  Order By reviewed_date DESC ")
    ...
    "where  ima.id in (select is_mgt_audit_id  from memorandom_contingent_mgt) AND  ima.approver_id= #{user_id} and ima.approve_status=1 order by ima.approved_date DESC")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
			@Result(property = "reviewer", column = "reviewer_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
	...
	@Select("select * from IS_Management_Audit where id in (SELECT is_mgt_audit_id from  memorandom_contingent_mgt) AND  review_status = 1 and approver_id=#{user_id} and status = 1 and approve_status = 2  Order By approver_rejected_date DESC")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "IS_MGTAuditee", column = "id", many = @Many(select = "com.afr.fms.Auditor.Mapper.AuditISMMapper.getISMAuditees")),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
