package com.afr.fms.Common.Report.Mapper;
...
@Mapper
public interface GenerateTaxReportMapper {
        ...
        @Select({
                        ...
                        "<where>",
                        "   tax.maker_id = #{user_id}", // <-- always filter by maker

                        // ✅ Router status logic
                        "<if test='router_status != null'>",
                        "   <choose>",
                        "       <when test='router_status == \"Drafted\"'>AND tax.status = 6</when>",
                        "       <when test='router_status == \"Waiting\"'>AND tax.status = 0</when>",
        ...
        @Select({
                        ...
                        "       <otherwise>AND tax.status IN (1, 3, 5)</otherwise>",
                        "   </choose>",
                        ...
                        "   <choose>",
                        "       <when test='rejected_date.size() == 2 and rejected_date[1] != null'>",
                        "           AND CAST(tax.approver_rejected_date AS DATE) BETWEEN CAST(#{rejected_date[0]} AS DATE) AND CAST(#{rejected_date[1]} AS DATE)",
                        "       </when>",
                        "       <otherwise>",
                        "           AND CAST(tax.approver_rejected_date AS DATE) = CAST(#{rejected_date[0]} AS DATE)",
                        "       </otherwise>",
                        "   </choose>",
                        "</if>",
