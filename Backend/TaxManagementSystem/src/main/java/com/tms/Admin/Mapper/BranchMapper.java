...
public interface BranchMapper {
	...
	@Select("WITH CTE AS (  " +
			...
			"SELECT *  " +
			"FROM CTE  " +
			"WHERE RowNum = 1 Order By name")
	@Results(value = {
			@Result(property = "id", column = "id"),

			@Result(property = "region", column = "region_id",

					one = @One(select = "com.tms.Admin.Mapper.RegionMapper.getRegionById"))
	})
	...
	@Select("WITH CTE AS (  " +
			...
			"SELECT *  " +
			"FROM CTE  " +
			"WHERE RowNum = 1 and status = 1 Order By name")
	@Results(value = {
			@Result(property = "id", column = "id"),
	...
	@Select("select top 1 * from branch where LTRIM(RTRIM(name)) = LTRIM(RTRIM(#{name}))")
	@Results(value = {
			@Result(property = "region", column = "region_id", one = @One(select = "com.tms.Admin.Mapper.RegionMapper.getRegionById")) })
