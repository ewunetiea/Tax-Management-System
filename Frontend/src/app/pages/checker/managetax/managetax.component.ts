...
getTaxes(): void {
  ...
  reviewSelectedTaxes(): void {
    this.selectedTaxes.forEach(tax => (tax.user_id = this.user.id));
  ...
  reviewTax(tax: Tax): void {
    tax.user_id = this.user.id;

    this.confirmationService.confirm({
      message: `Are you sure you want to review the tax <strong>${tax.reference_number}</strong>?`,
      header: 'Confirm Review',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
