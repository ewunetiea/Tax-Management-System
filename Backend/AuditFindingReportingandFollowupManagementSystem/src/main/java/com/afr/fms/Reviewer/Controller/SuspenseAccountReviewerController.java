...
import com.afr.fms.Reviewer.Service.SuspenseAccountReviewerService;
...
public class SuspenseAccountReviewerController {
	...
	private static final Logger logger = LoggerFactory.getLogger(SuspenseAccountReviewerController.class);
	...
	public ResponseEntity<List<AuditISM>> getFindingBasedOnStatus(@RequestBody PaginatorPayLoad paginatorPayload,
		...
		try {
			return new ResponseEntity<>(suspenseAccountReviewerService.getFindingBasedOnStatus(paginatorPayload),
					HttpStatus.OK);
		} catch (Exception ex) {
