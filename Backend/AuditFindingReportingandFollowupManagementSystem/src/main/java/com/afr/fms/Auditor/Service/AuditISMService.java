...
public class AuditISMService {
    ...
    public List<AuditISM> attachScheduledAuditeeResponse(List<AuditISM> auditISMs) {
        List<AuditISM> audits = new ArrayList<>();
        int response_count = 0;
        for (AuditISM auditISM : auditISMs) {
            List<IS_MGT_Auditee> is_MGT_Auditees = auditeeDivisionISMMapper.getScheduledISMAuditeesByAuditeeID(
                    auditISM.getDirectorate_id(),
                    auditISM.getId());
            response_count = 0;
            for (IS_MGT_Auditee is_MGT_Auditee : is_MGT_Auditees) {
                response_count += is_MGT_Auditee.getAuditeeDivisionISM() != null
                        ? is_MGT_Auditee.getAuditeeDivisionISM().size()
                        : 0;
            }
            if (response_count != 0) {
                auditISM.setIS_MGTAuditee(is_MGT_Auditees);
    ...
    public List<AuditISM> getAuditsOnDrafting(PaginatorPayLoad paginatorPayLoad) {
        return auditMapper.getAuditsOnDrafting(paginatorPayLoad);
    }

    public List<AuditISM> getAuditsOnProgressForAuditor(User auditor) {

        PageHelper.startPage(auditor.getPage_number(), auditor.getPage_size());
        Page<AuditISM> auditsPage = (Page<AuditISM>) auditMapper
                .getAuditsOnProgressForAuditor(auditor.getId());
    ...
    public void createISMFinding(AuditISM audit) {
        ...
        if (audit.getCategory().contains("IS")) {
    ...
    public void updateISMFinding(AuditISM audit) {
        ...
        if (audit.getEdit_auditee()) {
            auditMapper.deleteISMAuditee(audit.getId());
            IS_MGT_Auditee IS_MGTAuditee = new IS_MGT_Auditee();
    ...
    public void deleteISMFinding(Long id) {
        ...
        auditMapper.deleteISMFinding(id);
    ...
    public void deleteSelectedISMFinding(List<AuditISM> auditISMs) {
        for (AuditISM auditISM : auditISMs) {
            ...
            auditMapper.deleteISMFinding(auditISM.getId());
    ...
    public void passSelectedISMFinding(List<AuditISM> auditISMs) {
        for (AuditISM auditISM : auditISMs) {
            auditMapper.passISMFinding(auditISM.getId());
            AuditISM audit = auditMapper.getAudit(auditISM.getId());
            User user2 = new User();
            recentActivity = new RecentActivity();
            recentActivity
    ...
    public void backSelectedISMFinding(List<AuditISM> auditISMs) {
        for (AuditISM auditISM : auditISMs) {
            ...
            auditMapper.backISMFinding(auditISM.getId());
    ...
    public void changeRecitificationStatus(AuditISM auditISM) {
        auditMapper.changeRecitificationStatus(auditISM);
    }

    public List<AuditISM> checkFindingsExist(AuditISM finding) {
        List<AuditISM> audits = auditMapper.checkFindingsExist(finding.getFinding());
        return audits;
    }
