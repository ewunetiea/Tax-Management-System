...
public interface GenerateTaxReportMapper {
    ...
    @Select({
            ...
            "       <when test='router_status == \"Reject\"'>AND tax.status = 2</when>",
            "       <when test='router_status == \"Not Approved\"'>AND tax.status = 3</when>",
            "       <when test='router_status == \"Reviewed\"'>AND tax.status = 4</when>",
            "       <when test='router_status == \"Approved\"'>AND tax.status = 5</when>",
            "       <when test='router_status == \"Draft\"'>AND tax.status = 6</when>",
            "   </choose>",
            "</if>",
