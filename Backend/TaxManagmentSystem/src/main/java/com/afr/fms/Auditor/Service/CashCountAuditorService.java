...
import com.afr.fms.Auditor.Mapper.CashCountAuditorMapper;
...
@Service
public class CashCountAuditorService {
    @Autowired
    private CashCountAuditorMapper cashCountAuditorMapper;
    ...
    public void createCashCountFinding(AuditISM audit) {
        ...
        audit.getCashCount().setIs_mgt_audit_id(audit_id);
        cashCountAuditorMapper.createCashCount(audit.getCashCount());
    ...
    public void updateCashCountFinding(AuditISM audit) {
        User user = userMapper.getAuditorById(audit.getAuditor().getId());
        if (audit.getFinding_detail() == null) {
            cashCountAuditorMapper.updateMGTFinding(audit);
            cashCountAuditorMapper.updateCashCount(audit.getCashCount());
        } else {
            cashCountAuditorMapper.updateMGTFindingandFindingDetail(audit);
            cashCountAuditorMapper.updateCashCount(audit.getCashCount());
        }
    ...
    public List<AuditISM> getAuditsOnDrafting(PaginatorPayLoad paginatorPayload) {
        ...
        List<AuditISM> inspectionAudits = cashCountAuditorMapper.getAuditsOnDrafting(paginatorPayload);
    ...
    public List<AuditISM> getAuditsOnProgress(PaginatorPayLoad paginatorPayload) {
        // PageHelper.startPage(paginatorPayload.getCurrentPage(), paginatorPayload.getPageSize());
        List<AuditISM> inspectionAudits = cashCountAuditorMapper.getAuditsOnProgress(paginatorPayload);
        // if (!inspectionAudits.isEmpty()) {
        //     Page<AuditISM> page = (Page<AuditISM>) inspectionAudits;
        //     inspectionAudits.get(0).setTotal_records_paginator(page.getTotal());
        // }
    ...
    public List<AuditISM> getRejectedFindings(PaginatorPayLoad paginatorPayload) {
        // PageHelper.startPage(paginatorPayload.getCurrentPage(), paginatorPayload.getPageSize());
        List<AuditISM> inspectionAudits = cashCountAuditorMapper.getRejectedFindings(paginatorPayload);
        // if (!inspectionAudits.isEmpty()) {
        //     Page<AuditISM> page = (Page<AuditISM>) inspectionAudits;
        //     inspectionAudits.get(0).setTotal_records_paginator(page.getTotal());
        // }
